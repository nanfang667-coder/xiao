...
// 从表单里提取小巷子信息的文字字段
function extractAlleyFields(formData: FormData) {
  return {
    ...
    city: String(formData.get("city") ?? "").trim(),
    district: String(formData.get("district") ?? "").trim(),
    location: String(formData.get("location") ?? "").trim(), // 具体位置（会员可见）
  };
}

export async function createAlley(formData: FormData) {
  await requireAdmin();
  const fields = extractAlleyFields(formData);

  const files = formData.getAll("photos").filter((f): f is File => f instanceof File);
  const uploaded = await savePhotos(files);
  // 没上传图片就用一个默认占位色块
  const photos = uploaded.length > 0 ? uploaded : ["from-pink-300 to-rose-400"];

  await prisma.alley.create({
    data: { ...fields, photos: JSON.stringify(photos) },
  });

  revalidatePath("/alley");
  revalidatePath("/admin");
  revalidatePath("/admin/alleys");
  redirect("/admin/alleys");
}
...
export async function updateAlley(id: number, formData: FormData) {
  ...
  if (uploaded.length > 0) {
    ...
    const existing = await prisma.alley.findUnique({ where: { id } });
    try {
      photos = existing ? JSON.parse(existing.photos) : ["from-pink-300 to-rose-400"];
    } catch {
      photos = ["from-pink-300 to-rose-400"];
    }
  }

  await prisma.alley.update({
    where: { id },
    data: { ...fields, photos: JSON.stringify(photos) },
  });

  revalidatePath("/alley");
  revalidatePath(`/alley/${id}`);
  revalidatePath("/admin");
  revalidatePath("/admin/alleys");
  redirect("/admin/alleys");
}
